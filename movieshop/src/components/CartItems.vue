...
const incrementAmount = (item: ICart) => {
  item.amount++
}
  ...
  <table class="cart-table">
    ...
    <tbody>
      <tr v-for="item in cart" :key="item.productId">
        <td>
          <img :src="item.imageUrl" width="50" height="70" />
        </td>
        <td>{{ item.price }}:-</td>
        <td>
          <button class="cart-btn" @click.prevent="decrementAmount(item)">-</button>{{ item.amount
          }}<button class="cart-btn" @click.prevent="incrementAmount(item)">+</button>
        </td>
